...
(function () {
  ...
  var LANG = {
    pt: {
      ...
      favorite: 'Favorito', rename: 'Renomear', renamePrompt: 'Novo nome da conversa:',
      creativity: 'Criatividade', creaPrecise: 'Preciso', creaBalanced: 'Equilibrado', creaCreative: 'Criativo'
    },
    en: {
      ...
      favorite: 'Favourite', rename: 'Rename', renamePrompt: 'New conversation name:',
      creativity: 'Creativity', creaPrecise: 'Precise', creaBalanced: 'Balanced', creaCreative: 'Creative'
    },
    fr: {
      ...
      favorite: 'Favori', rename: 'Renommer', renamePrompt: 'Nouveau nom de la conversation :',
      creativity: 'Créativité', creaPrecise: 'Précis', creaBalanced: 'Équilibré', creaCreative: 'Créatif'
    }
  };
  ...
  loadCss('https://cdn.jsdelivr.net/npm/highlight.js@11.9.0/styles/github-dark.min.css');
  ...
  // Realce de sintaxe + botao "copiar" em cada bloco de codigo. Chamar quando a resposta termina.
  function enhanceCode(container) {
    if (!container) return;
    container.querySelectorAll('pre > code').forEach(function (code) {
      var pre = code.parentElement;
      if (pre.getAttribute('data-enh')) return;
      pre.setAttribute('data-enh', '1');
      if (window.hljs) { try { window.hljs.highlightElement(code); } catch (e) {} }
      var btn = document.createElement('button');
      btn.className = 'pcw-code-copy'; btn.type = 'button'; btn.textContent = getLang().copy;
      btn.addEventListener('click', function () {
        navigator.clipboard.writeText(code.innerText).then(function () {
          btn.textContent = getLang().copied;
          setTimeout(function () { btn.textContent = getLang().copy; }, 1500);
        });
      });
      pre.appendChild(btn);
    });
  ...
  var creativity = parseFloat(localStorage.getItem('pharus-crea'));
  if (isNaN(creativity)) creativity = 0.7; // default equilibrado
  ...
  function build() {
    ...
    var html = '<div id="pharus-chat">'
      ...
      +     '</button>'
      +   '</form>'
      +   '<div class="pcw-crea" id="pcw-crea">'
      +     '<span class="pcw-crea-lbl">' + t.creativity + '</span>'
      +     '<button type="button" data-c="0.2">' + t.creaPrecise + '</button>'
      +     '<button type="button" data-c="0.7">' + t.creaBalanced + '</button>'
      +     '<button type="button" data-c="1">' + t.creaCreative + '</button>'
      +   '</div>'
      +   '<div class="pcw-foot">'
    ...
    // Seletor de criatividade (temperature)
    var crea = document.getElementById('pcw-crea');
    if (crea) {
      var creaBtns = crea.querySelectorAll('button');
      function markCrea() {
        creaBtns.forEach(function (b) { b.classList.toggle('on', parseFloat(b.getAttribute('data-c')) === creativity); });
      }
      creaBtns.forEach(function (b) {
        b.addEventListener('click', function () {
          creativity = parseFloat(b.getAttribute('data-c'));
          localStorage.setItem('pharus-crea', String(creativity));
          markCrea();
        });
      });
      markCrea();
  ...
  function renderConvoList(list) {
    ...
    list.forEach(function (c) {
      ...
      fav.type = 'button';
      fav.className = 'pcw-convo-fav' + (c.favorite ? ' on' : '');
      fav.title = l.favorite;
      fav.innerHTML = c.favorite
        ? '<svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z"/></svg>'
        : '<svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14l-5-4.87 6.91-1.01L12 2z"/></svg>';
      fav.addEventListener('click', function (e) {
        e.stopPropagation();
        patchConvo(c.id, { favorite: !c.favorite }, openConvos);
      });
      ...
      ren.type = 'button';
      ren.className = 'pcw-convo-ren';
      ren.title = l.rename;
      ren.innerHTML = '<svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M12 20h9M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>';
      ren.addEventListener('click', function (e) {
        e.stopPropagation();
        var name = prompt(getLang().renamePrompt, c.title || '');
        if (name && name.trim()) patchConvo(c.id, { title: name.trim() }, openConvos);
      });
  ...
  // PATCH a uma conversa (renomear/favoritar) e recarregar a lista.
  function patchConvo(id, body, done) {
    body.sessionId = sessionId;
    fetch(API_BASE + '/api/conversation/' + encodeURIComponent(id), {
  ...
  async function sendMessage(message, opts) {
    ...
    try {
      var resp = await fetch(API_BASE + '/api/chat', {
        ...
        body: JSON.stringify({ sessionId: sessionId, conversationId: conversationId, message: message, lang: curLang(), regenerate: regenerate, file: fileToSend || undefined, webSearch: webSearchOn, temperature: creativity })
      });
